...
impl TextInputBox {
    fn on_add(mut world: DeferredWorld, context: HookContext) {
        let parent = world.entity(context.entity).get::<ChildOf>().unwrap().0;
        world
            .commands()
            .entity(parent)
...
pub fn select_text_input_box(
    trigger: On<Pointer<Over>>,
    mut commands: Commands,
    query: Query<(Entity, &ChildOf), With<TextInputBox>>,
) {
    for (entity, child_of) in query {
        if child_of.0 == trigger.entity {
            commands.entity(entity).insert(SelectedBox);
        }
    }
}

pub fn unselect_text_input_box(
    ...
    for (entity, child_of) in query {
        if child_of.0 == trigger.entity {
            commands.entity(entity).remove::<SelectedBox>();
        }
    }
}
